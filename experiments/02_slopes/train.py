"""
Slope-terrain AnymalC velocity-tracking training script.

Warm-starts from a flat-terrain checkpoint and fine-tunes for 1 000 iterations
on randomly inclined slopes (0–15 degrees), using only pyramid-slope sub-terrains
patched into the Rough env terrain generator.

Usage (run from /workspace/IsaacLab or anywhere):
    python /workspace/isaac-lab-locomotion/experiments/02_slopes/train.py --headless
    python /workspace/isaac-lab-locomotion/experiments/02_slopes/train.py --headless \\
        --load-checkpoint /workspace/checkpoints/flat/best_model.pt
...
parser = argparse.ArgumentParser(description="Train AnymalC on slope terrain with RSL-RL PPO.")
...
def build_runner_cfg(cfg: TrainingConfig) -> dict:
    ...
    return {
        ...
        "experiment_name": cfg.wandb_run_name or "slope_terrain",
...
def make_env(cfg: TrainingConfig) -> TensorDictVecEnvWrapper:
    """Create the Rough env with terrain generator patched to slope sub-terrains only."""
    import isaaclab.terrains as terrain_gen

    env_cfg = load_cfg_from_registry(cfg.env_id, "env_cfg_entry_point")
    env_cfg.scene.num_envs = cfg.num_envs

    # Replace mixed rough terrain with pyramid slopes only.
    # slope_range is in radians; 15° ≈ 0.2618 rad
    slope_max_rad = math.radians(cfg.terrain.slope_angle_max_deg)
    env_cfg.scene.terrain.terrain_generator.sub_terrains = {
        "hf_pyramid_slope": terrain_gen.HfPyramidSlopedTerrainCfg(
            proportion=0.5,
            slope_range=(math.radians(cfg.terrain.slope_angle_min_deg), slope_max_rad),
            platform_width=2.0,
            border_width=0.25,
        ),
...
def extract_slope_metrics(env: TensorDictVecEnvWrapper) -> dict:
    """
    Pull slope-specific diagnostics from env.extras["log"].

    Expected keys (populated by the terrain curriculum system):
      "slope_angle_deg"   — per-env current tile inclination in degrees
      "slip_events"       — per-env cumulative slip count this episode
    Falls back to terrain_difficulty proxy when those keys are absent.
    """
    metrics: dict = {}
    ...
    slope_angle = env_log.get("slope_angle_deg")
    if slope_angle is not None and slope_angle.numel() > 0:
        metrics["mean_slope_angle_encountered"] = slope_angle.float().mean().item()
    else:
        difficulty = env_log.get("terrain_difficulty")
        if difficulty is not None and difficulty.numel() > 0:
            metrics["mean_slope_angle_encountered"] = difficulty.float().mean().item() * 15.0  # 15° max

    slip_events = env_log.get("slip_events")
    if slip_events is not None and slip_events.numel() > 0:
        metrics["slip_events_per_episode"] = slip_events.float().mean().item()
...
def _partial_load_checkpoint(runner: OnPolicyRunner, path: str, device: str) -> None:
    """
    Load weights from *path* into *runner*, skipping any tensor whose shape
    differs between checkpoint and current model.

    This handles the flat→rough observation dimension mismatch (48 vs 235):
    the first MLP projection layer re-initialises, all hidden layers transfer.
    """
    ...
    for model_name, state_key in [
        ...
        for k, v in saved_sd.items():
            if k in current_sd and current_sd[k].shape == v.shape:
                filtered[k] = v
                transferred.append(f"{model_name}.{k}")
            else:
                skipped.append(
                    f"{model_name}.{k} {saved_sd[k].shape if k in saved_sd else '?'}" f" → {current_sd.get(k, '?')}"
                )
...
def train(cfg: TrainingConfig) -> None:
    ...
    for iteration in range(cfg.max_iterations):
        ...
        log_data.update(extract_slope_metrics(env))
        ...
        if iteration % cfg.log_interval == 0:
            slope_str = (
                f"  slope={log_data['mean_slope_angle_encountered']:.1f}°"
                if "mean_slope_angle_encountered" in log_data
                else ""
            )
            print(
                f"[iter {iteration + 1:>5}/{cfg.max_iterations}] "
                f"reward={mean_ep_reward:.3f}  best={best_reward:.3f}{slope_str}",
                flush=True,
            )
